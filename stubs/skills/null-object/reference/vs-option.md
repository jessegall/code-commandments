...
| Absence is a **value** the caller must read and may branch on, but is not an error | **`Option<T>`** (`{{ namespace }}\Option`) | A typed present-or-absent; caller uses `getOr()`/`map()`/`andThen()` — see the `commandments-option` skill. |
