...
final class StopHookBuilder
{
    ...
    private static function judgingHook(string $profile, ProfileOptions $opts): string
    {
        ...
        return self::header($profile, $b)
            ...
            # `--gate-probe` mode runs `judge` for its exit code only (no findings
            # report) even while a pilgrimage is active, so agents stay locked to the
            # guided walk yet the gate can still tell whether anything remains.
            ( cd "\$root" && eval "\${run}judge{$scopeFlag} --gate-probe" ) >/dev/null 2>&1
            status=\$?
