...
/**
 * Regenerates the auto-generated tables — the sins/detectors table (from
 * Detectors\Catalog, grouped by skill), the auto-fixing tables (the maintenance
 * Scribes plus every Repentable detector's fix), and the skills table. Each table
 * is written to its own standalone file (README.sins.md, README.scribes.md,
 * README.skills.md); the matching BEGIN/END block in README.md gets a one-line
 * excerpt linking to it. Run via `composer readme`.
 */
...
$engineSkills = static function (string $engine) use ($skillList, $shortName, $cell): array {
    ...
    foreach ($skillList as $skill) {
        if (str_starts_with($skill->slug, "{$engine}/")) {
            $lines[] = '| `' . $shortName($skill) . '` | ' . $cell((string) preg_replace('/\s+/', ' ', $skill->summary())) . ' |';
        }
    }
...
// ---- Write the table files + README excerpts -------------------------------
...
$tables = [
    'skills' => [
        'file' => 'README.skills.md',
        'title' => 'Skills',
        'block' => $skillsBlock,
        'excerpt' => '_' . count($skillList) . ' skills. Full table in [README.skills.md](README.skills.md)._',
    ],
    'detectors' => [
        'file' => 'README.sins.md',
        'title' => 'Sins & detectors',
        'block' => $detectorsBlock,
        'excerpt' => "_{$detectorTotal} sins across " . count($bySkill) . ' skills. Full tables in [README.sins.md](README.sins.md)._',
    ],
    'scribes' => [
        'file' => 'README.scribes.md',
        'title' => 'Auto-fixing',
        'block' => $scribesBlock,
        'excerpt' => '_`repent` auto-fixes ' . count($repentables) . ' sins, plus ' . count($maintenance) . ' whole-tree maintenance passes. Full tables in [README.scribes.md](README.scribes.md)._',
    ],
];
...
foreach ($tables as $marker => $table) {
    $path = dirname($readmePath) . '/' . $table['file'];
...
echo 'README regenerated (' . implode(', ', $written) . " — {$detectorTotal} detectors, " . count($repentables) . " auto-fixes).\n";
