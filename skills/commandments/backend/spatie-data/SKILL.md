...
The other 12 — one per rule — are in [`reference/examples.md`](reference/examples.md).

## Commands

- `vendor/bin/commandments judge --skill=backend/spatie-data` — find every one of these in the codebase.
- `vendor/bin/commandments info <sin>` — what one rule flags, why it is a sin, and the fix. The sins here: `all-nullable-data`, `all-optional-data`, `data-collection-type`, `data-method-hint-collision`, `hook-missing-computed`, `manual-hydration-loop`, `manual-input-cast`, `nested-type-missing-typescript`, `new-data-object`, `non-final-data`, `null-to-optional-map`, `nullable-wire-object`, `prefer-optional-create`.
- `vendor/bin/commandments repent --sin=<sin>` — auto-fix, for `all-nullable-data`, `data-collection-type`, `hook-missing-computed`, `manual-hydration-loop`, `nested-type-missing-typescript`, `new-data-object`, `non-final-data`, `prefer-optional-create`. Review it with `--dry-run` first.
- `vendor/bin/commandments scaffold --sin=<sin>` — generate the helper the fix reaches for, for `null-to-optional-map`.
- `vendor/bin/commandments report --detector=<Detector> --reason="…" --best-design="…" --ref=path:line` — the flagged code is CORRECT under the architecture and the rule is wrong. That is the only thing a report claims: a finding you agree with is yours to fix, however far the fix cascades.

## Reference

- [Worked examples](reference/examples.md) — every rule's bad → good, 13 of them.
