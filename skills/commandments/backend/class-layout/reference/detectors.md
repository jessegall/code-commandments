...
- **`member-after-method`** — A trait use, constant, property, property hook, or enum case declared below a method — so the reader only meets that state after seeing the behaviour that uses it. — `MemberAfterMethodDetector`
