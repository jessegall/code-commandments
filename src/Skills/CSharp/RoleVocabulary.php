...
or a `*Resolver` that does not dispatch, is a lie every caller pays for.";
    ...
    {
        return 'a keyed store / membership set / first-match dispatcher: name it `*Registry`/`*Set`/`*Resolver` and honour the contract — a registry `Get` throws on a miss.';
    }
...
PRINCIPLE;
    ...
    public function related(): array
    {
        return [
            ...
            Absence::class => 'a registry `Get` throws on a miss — the same "a must-exist thing that is missing throws" rule.',
            Exceptions::class => 'the named exception a registry throws on a miss, built by a static factory.',
        ];
    }
