...
final class CrossFileSet
{
    ...
    /**
     * The reading for this project, off disk when it still stands. Computed ONCE — at `install`/`sync`,
     * or on the first check that finds none — and re-read after that, so no edit pays for the parse.
     * The stamp falls when the package is upgraded or the project writes a rule of its own.
     */
    public static function forProject(Workspace $workspace): self
    {
        $file = $workspace->shared(self::FILE);
        ...
        File::write($file, (string) json_encode([
    ...
    /**
     * What the reading depends on: the engine's own source, and the rules the project wrote itself.
     * Either moving means the answer must be worked out again; nothing else can change it.
     */
    private static function stamp(Workspace $workspace): string
    {
        $stamps = [(string) @filemtime(self::packageSource())];
