...
| `bin/commandments plan [done\|stuck\|status]` | `done` ends the active plan — clears the worktree's `.plan-active` marker so the keep-going Stop hook stops nudging (the `executing-plans` skill runs it once the end gate is clean). `stuck` signals the agent is BLOCKED and needs the user: it pauses the keep-going nudge (writes `.plan-stuck` at HEAD) but keeps the plan ACTIVE — you may only `done` a COMPLETE plan, never a blocked one. The Stop hook stays silent while stuck at that HEAD and auto-clears the signal once HEAD moves (progress). **In the never-stop modes (`BestEffort`, `Relentless`) `stuck` REFUSES** — there is no waiting; the agent skips the blocker (BestEffort defers + retries at the end, Relentless just moves on). `status` reports whether a plan is active (and stuck), the resolved profile, and the `mode`. |
| `bin/commandments until "<condition>"` / `until list` / `until met <n>` / `until stuck` / `until pause` / `until resume` / `until clear` | The user's STOP GATE — plan-free and config-free: the user says "keep going until X" (or fires the published `/until` slash command) and the agent records X here ({@see Cli\Until\UntilGate}, session-scoped). While any condition stands, the `UntilReminder` Stop hook holds every stop and sends the agent back to VERIFY it (never assume); `met <n>` strikes a verified one off and the gate lifts when none remain, `stuck` releases ONE stop for a genuinely blocked agent while keeping the condition — and it is a claim about the WHOLE list, so the agent DRAINS every condition it can advance alone before asking (the hold message says so, and `stuck` names the others back at it). That claim is TESTED, not taken on trust (#419): the `PostToolUse` hook counts the WORK done under the gate — every tool use except talking to the gate itself and `TodoWrite` — each hold resets the count, and a `stuck` with ZERO work since that hold is REFUSED while several conditions stand (a lone condition is always releasable, so a blocked agent is never trapped). And the claim is GLOBAL, never local (#422): `stuck` is not "the thing in front of me needs the user" — it says NOT ONE standing condition can move — so it must carry `--blocked=` naming EVERY standing id, and anything left out is refused with the count and examples of the work still owed (the agent discovers, in composing the command, that its locally-true blocker is not the claim `stuck` makes). The claim itself is also CHALLENGED (#420): `stuck` must carry `--reason=` saying why NONE of them can move, and the gate puts it back twice — the first round names the shapes that are NOT blockers (low on context, too many call sites, a big or mechanical change, "this needs its own change"), the second asks it to be certain nothing on the list can move alone; the third call stands by the claim and prints it to the user. Any real work VOIDS a half-answered claim (the `PostToolUse` hook drops it), as does meeting a condition or pausing/resuming — a challenge is answered in one sitting or not at all. `pause`/`resume` are THE USER's switch — `pause` moves the marker to `.until.pause` so the whole gate (stop hold AND interjection nudge) goes quiet with every condition kept verbatim, `resume` puts it back (a pause holds NOTHING: a condition recorded while paused joins the set-aside ones and waits there too, so nothing can re-arm the gate behind the user's back) — and `clear` drops the gate (the user's call, never an escape hatch). Loop-safe: 10 consecutive held stops with no progress release the gate — which SETS THE CONDITIONS ASIDE (a `pause`, kept verbatim for the user to `resume`), never deletes them; meeting a condition resets that count. The gate also keeps the user's VISIBLE to-do list honest: `met` tells the agent to mark that item completed in `TodoWrite`, and 20 pieces of work with no update to it at all inject a "true it up" nudge — the list in the terminal is the one thing the user can check for themselves. The `commandments-until` skill teaches the discipline. Two disciplines ride on it: a mid-work interjection from the user is TRIAGED (steering the work in hand → do it now; a separate or deferred task → park it as a condition) via a `UserPromptSubmit` nudge that fires only while work is in flight, and an ACTIVE PLAN TAKES PRECEDENCE — the gate stays silent while `PlanReminder` owns the stop (and never burns its release cap), then takes over at `plan done`, which lists what now holds the agent. |
