...
| `repent` | Auto-fix findings marked `[AUTO-FIXABLE]` in the judge output — **both sins and warnings**. `[AUTO-FIXABLE]` declares a rewrite safe, which is orthogonal to severity, so an auto-fixable *warning* is repented without bumping the prophet to `sin` (which would also make it block commits). **Scope defaults to the git working tree** (the changed files, like `judge --git`) — a bare `repent` NEVER sweeps the scroll repo-wide. Widen explicitly with `--all` (whole scroll) or narrow with `--file`/`--files`; `--git` is the explicit form of the default. The commandments config file itself (`commandments.php`) is always skipped — it is tool config, not source under judgment. |
| `scripture [--prophet=NAME]` | Print the detailed rule behind a prophet. |
| `absolve` | Absolve a single finding — **warning OR sin**. Target it by `--fingerprint=<hash>` (copied from `judge --next`) OR by `--at=path:line` (or `--at=path:from-to`) — the same locator `judge` prints; `--at` resolves to the finding's content-based fingerprint at scan time (it stores no line number, so it self-heals when the file changes), and `--prophet=NAME` disambiguates ties on a line. Requires `--reason="…"`. A **single-target sin** absolution is a deliberate, **audited** escape (an absolved sin no longer blocks the gate) so an agent is never wedged by a pre-existing/out-of-scope sin it must not touch — FIX stays the default. Refuses unknown/stale fingerprints. `--all` **baselines the whole queue** (absolve every current advisory finding at once; sins still block). `--warnings` **batch-absolves every WARNING in scope** under one shared `--reason` (optionally narrowed with `--scope=git`/`--scope=staged`, or `--prophet=NAME` to scope to one prophet in a single scan); it **hard-refuses if any sin is in scope** and absolves nothing — no per-finding fingerprint walk. `--until-push` makes an absolution (single or `--warnings`) **sticky**: it survives the post-commit reset and stays until `git push` (the pre-push hook clears it via `--clear-until-push`) — for reasoned LEAVE warnings during an active grind, so the same coincidental warnings aren't re-litigated every commit. `--clear` **removes every ordinary absolution** — used by the post-commit hook so absolutions never silently persist across commits. |
...
| `install-hooks` | (Laravel) **Alias for `profile phased`.** Installs the phased bundle: Claude hooks (session-start scripture/briefing, a **Stop hook** — the fixed `.claude/hooks/stop-hook.sh`, **generated** from the profile's `ProfileBehaviour` (the `judge`/`test` cadence) by `StopHookBuilder` (the package ships no per-profile Stop stub): grind defers judge AND tests to the pre-push reckon and NEVER judges between phases, phased/sins-only gate on `judge --next --git`, penance on `judge --next`; the gate placement + per-phase nudge are DERIVED from the same behaviour; switching profiles regenerates the file so the settings entry never changes, and `disabled` removes it. A package update re-enters the active profile (via `sync`) so the script is regenerated — per-turn drift re-index, and a **PostToolUse** reminder after every `git commit`), a git **pre-commit gate** (blocks commits with sins), **post-commit reset**, **commit-msg guard**, **pre-push reset**, and a **`.gitignore` block** for the generated tracking state. Briefing is hook-delivered — it does NOT write a committed CLAUDE.md section. Idempotent. The standalone `init` does the same. |
...
| `--plan` | **Remediation roadmap (the penance plan).** Print EVERY finding in scope as a numbered checklist, ordered most-root-cause-first (the same order `--next` walks), each tagged `[Tier/Kind]` + `[AUTO-FIXABLE]`. Lets a cleanup pass see the whole path up front and fix root causes before symptoms (re-run as you go). Points at `repent` (bulk auto-fix) + `judge --next` (walk the rest). Exits non-zero while findings remain. |
