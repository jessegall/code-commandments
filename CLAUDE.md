...
| `bin/commandments until "<condition>"` / `until list` / `until met <n>` / `until blocked <id> --reason=` / `until stuck` / `until pause` / `until resume` / `until clear` | The user's STOP GATE — plan-free and config-free: the user says "keep going until X" (or fires the published `/until` slash command) and the agent records X here ({@see Cli\Until\UntilGate}, session-scoped). While any condition stands, the `UntilReminder` Stop hook holds every stop and sends the agent back to VERIFY it (never assume); `met <n>` strikes a verified one off and the gate lifts when none remain. `stuck` releases ONE stop for a genuinely blocked agent while keeping the conditions — and it is a claim about the WHOLE list, so the agent DRAINS every condition it can advance alone before asking. That claim is not ASSERTED, it is COUNTED, condition by condition (#419/#422): being blocked is a fact about ONE condition, so it is recorded ON that condition — `until blocked <id> --reason="<what only the user can give>"`, marked in whatever order the agent meets them ({@see Cli\Until\Condition} carries its own reason) — and `stuck` is exactly the moment NOT ONE standing condition is left unspoken-for. Anything unaccounted for is refused with the count and examples of the work still owed, so the agent discovers, in composing the claim, that its locally-true blocker ("the thing in front of me needs the user") is not what `stuck` says. The claim is also CHALLENGED twice (#420) before the stop is released — the first round names the shapes that are NOT blockers (low on context, too many call sites, a big or mechanical change, "this needs its own change"), the second asks it to be certain about the OTHERS; the third call stands by it and prints every reason to the user. A HELD STOP DROPS every block and any half-answered challenge, so a claim is always about the list as it stands then — never about what the agent said an hour ago. `pause`/`resume` are THE USER's switch — `pause` raises a flag so the whole gate (stop hold AND interjection nudge) goes quiet with every condition kept verbatim, `resume` puts it back (a pause holds NOTHING: a condition recorded while paused waits with the set-aside ones, so nothing can re-arm the gate behind the user's back) — and `clear` drops the gate (the user's call, never an escape hatch). Loop-safe: 10 consecutive held stops with no progress release the gate — which SETS THE CONDITIONS ASIDE (a `pause`, kept verbatim for the user to `resume`), never deletes them; meeting a condition resets that count. The gate also keeps the user's VISIBLE to-do list honest: `met` tells the agent to mark that item completed in `TodoWrite`, and 20 pieces of work with no update to it at all inject a "true it up" nudge — the list in the terminal is the one thing the user can check for themselves. It must also be READABLE at a glance: every `TodoWrite` under the gate is checked ({@see Hooks\TodoList}) and the agent is sent back to reorder when the list does not LEAD with the item in progress, so the first line always answers "what is it doing right now?". The `commandments-until` skill teaches the discipline. Two disciplines ride on it: a mid-work interjection from the user is TRIAGED (steering the work in hand → do it now; a separate or deferred task → park it as a condition) via a `UserPromptSubmit` nudge that fires only while work is in flight, and an ACTIVE PLAN TAKES PRECEDENCE — the gate stays silent while `PlanReminder` owns the stop (and never burns its release cap), then takes over at `plan done`, which lists what now holds the agent. |
