...
1. Record it: run `vendor/bin/commandments stop-condition "$ARGUMENTS"` (keep the user's wording; rephrase
...
2. Surface it: add the SAME condition to your to-do list (TodoWrite) as a pending item, so the user
   ...
   `stop-condition met <n>`, and keep whatever you are working on at the TOP of the list, so its first line
   always says where you are.
3. Load the `commandments-stop-condition` skill (Skill tool) and follow it — it is the discipline for working
   under a gate.
4. Get to work on making the condition hold. From here every stop you attempt is held by the Stop
   ...
   command / read the output — never assume), run `vendor/bin/commandments stop-condition met <n>`. If you
   are genuinely blocked, run `vendor/bin/commandments stop-condition stuck` and tell the user what blocks
   you. Never `stop-condition clear` to escape a condition you haven't met.
...
every question at once). `stop-condition stuck` claims the WHOLE list is blocked.
