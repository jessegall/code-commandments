...
final class ChecklistTest extends TestCase
{
    ...
    public function test_clear_all_removes_the_live_file_and_its_archives(): void
    {
        ...
        file_put_contents(Workspace::at($this->dir)->path('sins-2026-07-04_101112.md'), "old\n");
        file_put_contents(Workspace::at($this->dir)->path('sins-2026-07-03_090807.md'), "older\n");

        Checklist::inSession(Workspace::at($this->dir))->clearAll();

        $this->assertFileDoesNotExist($this->live());
        $this->assertCount(0, glob(Workspace::at($this->dir)->path('sins*.md')) ?: [], 'archives are gone too');
    }

    private function live(): string
    {
        return Workspace::at($this->dir)->path('sins.md');
    }
}
