 ...
 * `judge-reminder` under `Stop` — from ANY starting state, including a stale/duplicate copy under the
 * wrong event, while never touching a hook it didn't write. It runs on every `composer update` (via
 * sync), so it has to be idempotent by content.
 */
