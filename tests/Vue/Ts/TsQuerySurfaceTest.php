...
use JesseGall\CodeCommandments\Vue\ExprMatch;
use JesseGall\CodeCommandments\Vue\TsMatch;
 ...
 */
final class TsQuerySurfaceTest extends TestCase
{
    ...
    public function test_declarations_are_reachable_with_their_file_and_line(): void
    {
        ...
        $this->assertSame(['client/protocol.ts:1'], array_map(static fn (TsMatch $m): string => $m->location(), $classes));
        ...
        $this->assertSame(['send', 'debounce'], array_map(static fn (TsMatch $m): string => $m->name(), $functions));
    }

    public function test_parameters_and_fields_are_reachable(): void
    {
        $this->assertSame(['payload', 'ms'], array_map(
            static fn (TsMatch $m): string => $m->name(),
    ...
    public function test_statements_inside_a_method_body_are_reachable(): void
    {
        $this->assertSame(['client/protocol.ts:5'], array_map(
            static fn (TsMatch $m): string => $m->location(),
    ...
    public function test_a_real_rule_composes_the_way_a_backend_one_does(): void
    {
        $hits = $this->codebase()
            ...
            ->where(static fn (ExprMatch $match): bool => $match->expr->argument(0)?->is(\JesseGall\CodeCommandments\Vue\Expr\ExprKind::Literal) === true)
            ->get();
