...
class PreferOptionFactoryProphetTest extends TestCase
{
    ...
    public function test_flags_some_none_ternary_and_suggests_when(): void
    {
        // The reported case.
...
PHP);
        ...
        $this->assertStringContainsString('Option::when($this->instance->has($nodeId), fn () => $this->instance->node($nodeId)->descriptor)', $judgment->warnings[0]->message);
    }

    public function test_flags_make_wrapping_a_null_ternary_and_suggests_when(): void
    {
...
PHP);
    ...
    public function test_make_with_null_on_the_true_branch_suggests_whenNot(): void
    {
        $judgment = $this->judge('public function f(): Option { return Option::make($hidden ? null : $value); }');

        $this->assertCount(1, $judgment->warnings);
        $this->assertStringContainsString('Option::whenNot($hidden, fn () => $value)', $judgment->warnings[0]->message);
    }
...
PHP);
        ...
        $this->assertCount(1, $judgment->warnings);
        $this->assertStringContainsString('Option::when($this->reg->has($id), fn () => $this->reg->get($id))', $judgment->warnings[0]->message);
    }

    public function test_some_on_the_false_branch_suggests_whenNot(): void
    {
        $judgment = $this->judge('public function f($cond, $v): Option { return $cond ? Option::none() : Option::some($v); }');

        $this->assertCount(1, $judgment->warnings);
        $this->assertStringContainsString('Option::whenNot($cond, fn () => $v)', $judgment->warnings[0]->message);
    }
